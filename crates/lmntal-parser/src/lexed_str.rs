...
use lmntal_lexer::unescape::{EscapeError, Mode};
...
impl<'a> LexedStr<'a> {
    pub fn new(text: &'a str) -> LexedStr<'a> {
        ...
        for token in lmntal_lexer::tokenize(&text[conv.offset..]) {
    ...
    pub fn single_token(text: &'a str) -> Option<(SyntaxKind, Option<String>)> {
        ...
        let token = lmntal_lexer::tokenize(text).next()?;
...
impl<'a> Converter<'a> {
    ...
    fn extend_token(&mut self, kind: &lmntal_lexer::TokenKind, token_text: &str) {
        ...
        let syntax_kind = {
            match kind {
                lmntal_lexer::TokenKind::LineComment {} => Comment,
                lmntal_lexer::TokenKind::BlockComment { terminated } => {
                    if !terminated {
                ...
                lmntal_lexer::TokenKind::Whitespace => Whitespace,

                lmntal_lexer::TokenKind::Ident if token_text == "_" => Underscore,
                lmntal_lexer::TokenKind::Ident => SyntaxKind::from_keyword(token_text)
                    .or(SyntaxKind::from_reserved_operator(token_text))
                    .or(SyntaxKind::from_text(token_text))
                    .unwrap_or_else(|| {
                        err = "Unknown keyword";
                        AtomName
                    }),

                lmntal_lexer::TokenKind::Literal { kind, .. } => {
                    self.extend_literal(token_text.len(), kind);
                    return;
                }

                lmntal_lexer::TokenKind::Semi => T![;],
                lmntal_lexer::TokenKind::Comma => T![,],
                lmntal_lexer::TokenKind::Dot => T![.],
                lmntal_lexer::TokenKind::OpenParen => T!['('],
                lmntal_lexer::TokenKind::CloseParen => T![')'],
                lmntal_lexer::TokenKind::OpenBrace => T!['{'],
                lmntal_lexer::TokenKind::CloseBrace => T!['}'],
                lmntal_lexer::TokenKind::OpenBracket => T!['['],
                lmntal_lexer::TokenKind::CloseBracket => T![']'],
                lmntal_lexer::TokenKind::At => T![@],
                lmntal_lexer::TokenKind::Pound => T![#],
                lmntal_lexer::TokenKind::Tilde => T![~],
                lmntal_lexer::TokenKind::Question => T![?],
                lmntal_lexer::TokenKind::Colon => T![:],
                lmntal_lexer::TokenKind::Dollar => T![$],
                lmntal_lexer::TokenKind::Eq => T![=],
                lmntal_lexer::TokenKind::Bang => T![!],
                lmntal_lexer::TokenKind::Lt => T![<],
                lmntal_lexer::TokenKind::Gt => T![>],
                lmntal_lexer::TokenKind::Minus => T![-],
                lmntal_lexer::TokenKind::And => T![&],
                lmntal_lexer::TokenKind::Or => T![|],
                lmntal_lexer::TokenKind::Plus => T![+],
                lmntal_lexer::TokenKind::Star => T![*],
                lmntal_lexer::TokenKind::Slash => T![/],
                lmntal_lexer::TokenKind::Caret => T![^],
                lmntal_lexer::TokenKind::Percent => T![%],
                lmntal_lexer::TokenKind::Backslash => T!['\\'],
                lmntal_lexer::TokenKind::Unknown => Error,
                lmntal_lexer::TokenKind::Eof => EOF,
            }
        };
    ...
    fn extend_literal(&mut self, len: usize, kind: &lmntal_lexer::LiteralKind) {
        let mut err = "";

        let syntax_kind = match *kind {
            lmntal_lexer::LiteralKind::Int { empty_int, base: _ } => {
                if empty_int {
                    err = "Missing digits after the integer base prefix";
                }
                IntNumber
            }
            lmntal_lexer::LiteralKind::Float {
                empty_exponent,
                base: _,
            } => {
                if empty_exponent {
            ...
            lmntal_lexer::LiteralKind::Char { terminated } => {
                if !terminated {
                    ...
                    let text = &text[..i];
                    if let Err(e) = lmntal_lexer::unescape::unescape_char(text) {
                        err = error_to_diagnostic_message(e, Mode::Char);
                    }
                }
                Char
            }
            lmntal_lexer::LiteralKind::Str { terminated } => {
                if !terminated {
...
fn unescape_string_error_message(text: &str, mode: Mode) -> &'static str {
    ...
    match mode {
        Mode::CStr => {
            lmntal_lexer::unescape::unescape_mixed(text, mode, &mut |_, res| {
                if let Err(e) = res {
        ...
        Mode::ByteStr | Mode::Str => {
            lmntal_lexer::unescape::unescape_unicode(text, mode, &mut |_, res| {
                if let Err(e) = res {
                    error_message = error_to_diagnostic_message(e, mode);
                }
            });
